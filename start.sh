...
if [ ! -s "$PROBE_CACHE" ]; then
    echo "[start.sh] Pre-populating probe cache..."
    python3 -c "
...
known = [
    ...
    'CS.D.GBPJPY.MINI.IP', 'CS.D.AUDJPY.MINI.IP', 'CS.D.EURCHF.MINI.IP',
    'CS.D.CADJPY.MINI.IP', 'CS.D.AUDCAD.MINI.IP', 'CS.D.AUDNZD.MINI.IP',
    'CS.D.CADCHF.MINI.IP', 'CS.D.CHFJPY.MINI.IP', 'CS.D.EURAUD.MINI.IP',
    'CS.D.EURSEK.MINI.IP', 'CS.D.EURNOK.MINI.IP', 'CS.D.EURTRY.MINI.IP',
    'CS.D.USDTRY.MINI.IP', 'CS.D.USDMXN.MINI.IP',
    'CS.D.ETHEREUM.CFBMU.IP', 'CS.D.BITCOIN.CFBMU.IP',
    'CS.D.BITCOIN.CFD.IP', 'CS.D.ETHEREUM.CFD.IP',
    'CS.D.IN_GOLD.MFI.IP', 'CS.D.IN_SILVER.MFI.IP', 'CS.D.COPPER.MINI.IP',
    'CC.D.CL.USS.IP', 'IX.D.SPTRD.DAILY.IP', 'IX.D.FTSE.DAILY.IP',
...
try:
    ...
    for epic in get_universe_epics():
        if epic not in results:
            results[epic] = {'available': False, 'name': epic, 'reason': 'not_tested'}
except Exception:
    pass
cache = {'ts': time.time() - 60, 'results': results}
with open('$PROBE_CACHE', 'wb') as f:
    pickle.dump(cache, f)
print(f'Wrote {len(results)} EPICs to cache')
" 2>&1 | tail -2
fi
